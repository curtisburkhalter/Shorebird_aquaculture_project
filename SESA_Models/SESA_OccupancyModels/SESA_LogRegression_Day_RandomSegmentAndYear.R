...
NSESA.new <- read.delim(file=here("Shorebird_aquaculture_project","InitialValueFiles","NSESA.newPA_inits.txt"),header=TRUE)
names(NSESA.new) <- NULL
NSESA.new <- as.integer(NSESA.new[,1])

eps <- read.delim(file=here("Shorebird_aquaculture_project","InitialValueFiles","epsPA.txt"),header=TRUE)
names(eps) <- NULL
eps <- as.integer(eps[,1])

comdata <- read.delim(file=here("Shorebird_aquaculture_project","DataForModelFitting","SESA_fittingPA032118.txt"),header=T)
comdata$time <- NULL
...
comdata$day <- rescale(comdata$day)
...
initsFunction = function() {list(Beta_day=Beta_day,
...
data <- list('N','sesa_occupancy','year','segment','TS','nGulls',
...
writeLines("
          ...
          # y year random effect
           ...
           sesa_occupancy[i] ~ dbern(p.occ[i]) 
           
           logit(p.occ[i]) <- alpha.lam + Beta_day*day[i] + seg_rand[segment[i]] + year_rand[year[i]]
           
           # Fit assessments
           
           residual[i] <- abs(sesa_occupancy[i] - p.occ[i])
           
           
           # Generate replicate datasets
           NSESA.new[i] ~ dbern(p.occ[i])
           residual.new[i] <- abs(NSESA.new[i]-p.occ[i])
           ...
           ", con = here("Shorebird_aquaculture_project","SESA_Models","SESA_OccupancyModels", "LogRegression_DayRandomSegmentandYearModel.txt"))

#Identify filepath of model file;
modfile <- here("Shorebird_aquaculture_project","SESA_Models", "SESA_OccupancyModels", "LogRegression_DayRandomSegmentandYearModel.txt")
...
sink(file=here("Shorebird_aquaculture_project","OutputFiles","SESA","outputLogRegression_Day_RandomSegmentandYearModel.txt"))
