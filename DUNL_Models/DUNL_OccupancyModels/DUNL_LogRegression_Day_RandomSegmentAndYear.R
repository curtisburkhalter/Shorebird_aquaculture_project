...
NDUNL.new <- read.delim(file=here("Shorebird_aquaculture_project","InitialValueFiles","NDUNL.newPA_inits.txt"),header=TRUE)
names(NDUNL.new) <- NULL
NDUNL.new <- as.integer(NDUNL.new[,1])

eps <- read.delim(file=here("Shorebird_aquaculture_project","InitialValueFiles","epsPA.txt"),header=TRUE)
names(eps) <- NULL
eps <- as.integer(eps[,1])

comdata <- read.delim(file=here("Shorebird_aquaculture_project","DataForModelFitting","DUNL_fittingPA032118.txt"),header=T)
comdata$time <- NULL
...
data <- list('N','dunl_occupancy','year','segment','TS','nGulls',
...
writeLines("
          ...
          # y year random effect
           ...
           ", con = here("Shorebird_aquaculture_project","DUNL_Models","DUNL_OccupancyModels", "LogRegression_DayRandomSegmentandYearModel.txt"))

#Identify filepath of model file;
modfile <- here("Shorebird_aquaculture_project","DUNL_Models", "DUNL_OccupancyModels", "LogRegression_DayRandomSegmentandYearModel.txt")
...
sink(file=here("Shorebird_aquaculture_project","OutputFiles","DUNL","outputLogRegression_Day_RandomSegmentandYearModel.txt"))
