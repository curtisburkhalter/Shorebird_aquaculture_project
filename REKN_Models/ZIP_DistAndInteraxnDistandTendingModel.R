...
#number of levels of distance class
n.class <- 7

#number of levels of tending
n.tending <- 2

#initial values for parameters
group.mean=matrix(data=rep(1,times=14),nrow=7,ncol=2,byrow=TRUE)
omega=1
tau.disp=3
...
initsFunction = function() {list(Beta_FEDR=Beta_FEDR, alpha.lam=alpha.lam, group.mean=group.mean,omega=omega,tau.disp=tau.disp,eps=eps,w=w)}

#package the data to be used in JAGS by providing the dataset names
data <- list('n.class','n.tending', 'N','REKN','year','FEDR','Nsegment','NWD','time','AT','windS','TS','nGulls',
...
params<-c("alpha.lam","Beta_FEDR","group.mean","fit","fit.new","omega","tau.disp","bpvalue")
...
writeLines("
           ...
           for (i in 1:n.class) {
                for (j in 1:n.tending) {
                  group.mean[i,j]~dnorm(0,0.1)
                }
           }
           ...
           ", con = here("Shorebird_aquaculture_project","REKN_Models", "ZIP_InteraxnDistandTendingModel.txt"))

#Identify filepath of model file;
modfile <- here("Shorebird_aquaculture_project","REKN_Models", "ZIP_InteraxnDistandTendingModel.txt")
