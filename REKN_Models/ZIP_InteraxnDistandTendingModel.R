...
initsFunction = function() {list(group.mean=group.mean,omega=omega,tau.disp=tau.disp,eps=eps,w=w)}
...
params<-c("group.mean","fit","fit.new","omega","tau.disp","bpvalue")
...
writeLines("
           ...
           log(lambda[i]) <- group.mean[FEDR[i],tending[i]] + eps[i]
