...
w <- read.delim(file=here("Shorebird_aquaculture_project","InitialValueFiles","wPA.txt"),header=TRUE)
names(w) <- NULL
w <- as.integer(w[,1])
...
writeLines("
           ...
           w[i] ~ dbern(omega)
           rekn_occupancy[i] ~ dbinom(eff.p.occ[i],N) 
           eff.p.occ[i] <- w[i]*p.occ[i]
